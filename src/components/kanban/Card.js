...
const TicketCard = ({ 
	...
	const handleCardDelete = async () => {
		...
		const {data} = await deleteCard(content._id);
		if(data){ //if card is deleted only then
			console.log(data);
			let newArr = cards.filter(card => card._id !== content._id);
			setCards(cards);
	...
	function viewDetails(){
		history.push(`/cards/cardprofile/${content._id}`,{data: content._id});
	  }
		...
		<div className="mb-2 mt-2 ticketCardWidth">
			...
			>
				...
				<CardFooter className="text-muted f-w-600">
					<Router>
						{/* <Link
							// to={`/cards/cardprofile/${content._id}`}
							to={{
								pathname: `/cards/cardprofile/${content._id}`,
								state: {
									data: content._id,
								},
							}}
