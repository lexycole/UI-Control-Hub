...
import { getMechanics } from "./../../services/mechanics";
import "bootstrap/dist/css/bootstrap.min.css";
//import FloatSubMenu from './../../components/float-sub-menu/float-sub-menu';
import Pagination from "../../common/pagination";
import { paginate } from "../../utils/paginate";
import MechanicsTable from "../../components/mechanicsTable.jsx";
...
class MechanicsTableData extends Component {
	constructor(props) {
		super(props);
		this.state = {
			mechanics: [],
	...
	async componentDidMount() {
		//const {data:mechanics} = await axios.get("http://localhost:4500/api/mechanics");
		const data = await getMechanics();
		console.log(data.data);
		this.setState({ mechanics: data.data });
	}

	handleDelete = (user) => {
		console.log(user);
		const mechanics = this.state.mechanics.filter((el) => el._id !== user._id);
		this.setState({ mechanics: mechanics });
	};


	handleMassDelete = (CheckedFields) => {
		const originalMechanics = this.state.mechanics;
		CheckedFields.map(async (mechanic) => {
			const mechanics = this.state.mechanics.filter((Mechanic) => Mechanic._id !== mechanic);
		
			this.setState({ mechanics });
			try {
				await http.delete(apiUrl + "/mechanics/" + mechanic);
			} catch (ex) {
				if (ex.response && ex.response === 404) {
					alert("already deleted");
				}

				this.setState({ mechanics: originalMechanics });
			}
			console.log("Users: ", this.state.mechanics);
		});
	};
...
handleCheckboxChange = ({ target: { checked, value } }) => {
	...
	getDataPgnation = () => {
		const { pageSize, currentPage, mechanics: Mechanics, sortColumn, searchQuery } = this.state;
		//
		//filter maybe next time
		let filtered = Mechanics;
		if (searchQuery) {
			console.log(searchQuery);
			filtered = Mechanics.filter(
				(el) =>
		...
		const mechanics = paginate(sorted, currentPage, pageSize);
		return { data: mechanics };
	};

	render() {
		const { length: count } = this.state.mechanics;
		const { pageSize, currentPage, sortColumn, searchQuery } = this.state;
		// if(count === 0)  return "<p>No data available</p>";

		const { data: mechanics } = this.getDataPgnation();
		...
		return (
			<div>
				...
				<h1 className="page-header">Mechanics </h1>
				<Panel>
					<PanelHeader>Mechanics Management</PanelHeader>

					<React.Fragment>
						<ToastContainer />
						<div className="toolbar" style={toolbarStyles}>
							<button className="btn btn-default active m-r-5 m-b-5" title="add mechanic" style={btnStyles}>
								{" "}
								<Link to="/clinic/mechanics/new">
									<img style={iconStyles} src={newIcon} />
								</Link>
							</button>

							<button className="btn btn-default active m-r-5 m-b-5" title="edit mechanic" style={btnStyles}>
								{" "}
								<Link
									to={
										this.state.checkedFields
											? `/clinic/mechanics/${this.state.checkedFields[0]}`
											: "/clinic/mechanics/"
									}
								>
...
export default MechanicsTableData;