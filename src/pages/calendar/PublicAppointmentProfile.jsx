...
import {
    TabContent, Button, Form, FormGroup, Label, Input, Table, Row, Col, Modal,
    ModalBody,
    ModalFooter,
    ModalHeader,
} from "reactstrap";
...
class PublicAppointmentProfile extends React.Component {
    constructor(props) {
        super(props);
        ...
        this.togglePermissionDialog = this.togglePermissionDialog.bind(this);
        this.toggleInputDialog = this.toggleInputDialog.bind(this);
        this.checkNonRegisteredUser = this.checkNonRegisteredUser.bind(this);
    ...
    async PopulateCurrentUser() {
        try {
            ...
            if (user) {
                console.log("SHARED USERS", this.state.data.sharedUsers)
                let index = this.state.data.sharedUsers.findIndex(shareduser => shareduser.userid === user._id)
                console.log("index", index)
                if (index === -1) {
                    this.togglePermissionDialog()
                }
            } else {
    ...
    togglePermissionDialog() {
        const { permissionDialog } = this.state;
        this.setState({ permissionDialog: !permissionDialog });
    }

    toggleInputDialog() {
        const { inputDialog } = this.state;
        this.setState({ inputDialog: !inputDialog });
    }

    checkNonRegisteredUser = (e) => {
        e.preventDefault()
        let index = this.state.data.sharedNonregistredUsers.findIndex(shareduser => shareduser.email === this.state.nonRegisterMailInput)
        console.log("index", index)
        if (index === -1) {
    ...
    render() {
        ...
        if (this.state.permissionDialog) {
            return (
                <Modal isOpen={this.state.permissionDialog} >
                    ...
                    <ModalFooter>
                        <Button onClick={() => this.props.history.goBack()} color="primary" variant="outlined">
                            Go Back
                        </Button>
                    </ModalFooter>
                </Modal>)
            {/*<Dialog
                    open={this.state.permissionDialog}
                >
                    <DialogTitle>
                        Delete Appointment
        </DialogTitle>
        ...
        if (this.state.inputDialog) {
            return (
                <Modal isOpen={this.state.inputDialog} >
                    {/*<ModalHeader>Rename </ModalHeader>*/}
                    <ModalBody>
                        <Form onSubmit={this.checkNonRegisteredUser}>
                            <FormGroup>
                                <Label for="exampleEmail">Enter your email to access</Label>
                                <Input
                                    type="text"
                                    name="nonRegisterMailInput"
                                    value={this.state.nonRegisterMailInput}
                                    onChange={(e) => this.setState({ nonRegisterMailInput: e.target.value })}
...
export default PublicAppointmentProfile;