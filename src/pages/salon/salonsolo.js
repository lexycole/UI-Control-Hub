...
import { saveSalon, getSalon } from "./../../services/salons";
const createSliderWithTooltip = Slider.createSliderWithTooltip;
const Handle = Slider.Handle;

class SalonSolo extends Form {
  constructor(props) {
    super(props);
  ...
  async populateUser() {
    try {
      ...
      const { data: user } = await getSalon(userId);
      //console.log(user[0]);
      //const clinic = Object.assign({},user[0]);
      const salon = user[0];
      //console.log(clinic);
      if (salon.dateBirth) salon.dateBirth = new Date(salon.dateBirth);
      else salon.dateBirth = new Date();
      if (salon.professionalInfo.licenseValidTill)
        salon.licenseValidTill = new Date(
          salon.professionalInfo.licenseValidTill
        );
      else salon.licenseValidTill = new Date();
      salon.firstName = salon.contactName.first;
      salon.lastName = salon.contactName.last;
      salon.initials = salon.contactName.initials;
      salon.IBAN = salon.bankInfo.IBAN;
      salon.bank = salon.bankInfo.bank;
      salon.branchOfBank = salon.bankInfo.branchOfBank;
      salon.subscription = salon.subscription.subscription;
      salon.subscriptionEndDate = salon.subscription.subscriptionEndDate;
      salon.businessName = salon.companyInfo.businessName;
      salon.chamberCommerceNo = salon.companyInfo.chamberCommerceNo;
      salon.taxPayerNo = salon.companyInfo.taxPayerNo;
      salon.website = salon.companyInfo.website;
      salon.size = salon.companyInfo.size;
      salon.industry = salon.companyInfo.industry;
      salon.treatments = salon.professionalInfo.treatments;
      salon.licenseNo = salon.professionalInfo.licenseNo;
      salon.organizationAName = salon.membership.organizationAName;
      salon.organizationAMemberNo = salon.membership.organizationAMemberNo;
      salon.organizationBName = salon.membership.organizationBName;
      salon.organizationBMemberNo = salon.membership.organizationBMemberNo;
      salon.longitude = salon.companyInfo.location.coordinates[0];
      salon.latitude = salon.companyInfo.location.coordinates[1];

      //  delete salon.password;

      this.setState({ data: this.mapToViewModel(salon) });
      console.log(salon);
  ...
  doSubmit = async (user) => {
    ...
    try {
      console.log(this.state.data);
      await saveSalon(this.state.data, this.state.imageSrc);
      //console.log(this.state.data);
      this.props.history.push("/salon/salonsolos");
    } catch (ex) {
      //if(ex.response && ex.response.status === 404){
      if (ex.response) {
        const errors = { ...this.state.errors };
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <div>
          <ol className="breadcrumb float-xl-right">
            ...
            <li className="breadcrumb-item">
              <Link to="/salon/salonsolos">Salons</Link>
            </li>
            <li className="breadcrumb-item active">Add Salon</li>
          </ol>
          <h1 className="page-header">
            Add Salon-Solo <small>Salon-Solo-registration-form</small>
          </h1>

          <div className="row">
            <div className="col-xl-10">
              <Panel>
                <PanelHeader>Add Salon</PanelHeader>
                      ...
                      <div className="btn-group col-lg-8">
                        <div className="custom-control custom-radio custom-control-inline">
                          <input
                            ...
                            value="Salon"
                            checked={data.subscription === "Salon"}
                          />
                          <label
      ...
      <RadioGroup row aria-label="position" name="subscription" defaultValue="top">
        ...
        <FormControlLabel value="Salon" control={<Radio color="primary" />} label="Salon" labelPlacement="center" onChange={this.handleChange} value="Salon"  checked={data.subscription === "Salon" }  />
      </RadioGroup>
	  ...
	  {errors.subscription && (<div className="alert alert-danger">{errors.subscription}</div>)}
                    ...
                    <div className="form-group row">
                      ...
                      <div className="col-lg-8">
                        <DatePicker
                          onChange={this.licenseValidTillChange}
...
export default withRouter(SalonSolo);
