...
import { getCompanys, deleteCompany } from "./../../services/companies";
...
class CompanyTable extends Component {
  ...
  handleDelete = async (user) => {
    ...
    this.setState({ users });
    try {
      await http.delete(apiUrl + "/companysolo/" + user._id);
    } catch (ex) {
      //ex.request
      //ex.response
    ...
    return (
      <div>
        <ol className="breadcrumb float-xl-right">
          ...
          <li className="breadcrumb-item active">Companies Table</li>
        </ol>
        <h1 className="page-header">Companies </h1>
        <Panel>
          <PanelHeader>Companies Management</PanelHeader>
