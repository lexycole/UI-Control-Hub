...
import { getMures } from "./../../../services/mures";
import "bootstrap/dist/css/bootstrap.min.css";
//import FloatSubMenu from './../../components/float-sub-menu/float-sub-menu';
import Pagination from "../../../common/pagination";
import { paginate } from "../../../utils/paginate";
import MuresTable from "../../../components/muresTable.jsx";
...
class MuresTableData extends Component {
	constructor(props) {
		super(props);
		this.state = {
			mures: [],
	...
	async componentDidMount() {
		//const {data:Mures} = await axios.get("http://localhost:4500/api/Mures");
		const data = await getMures();
		console.log(data.data);
		this.setState({ mures: data.data });
	}

	handleDelete = (user) => {
		console.log(user);
		const Mures = this.state.mures.filter((el) => el._id !== user._id);
		this.setState({ mures: Mures });
	};


	handleMassDelete = (CheckedFields) => {
		const originalMures = this.state.mures;
		CheckedFields.map(async (Mure) => {
			const Mures = this.state.mures.filter((Mure) => Mure._id !== Mure);
		
			this.setState({ mures:Mures });
			try {
				await http.delete(apiUrl + "/Mures/" + Mure);
			} catch (ex) {
				if (ex.response && ex.response === 404) {
					alert("already deleted");
				}

				this.setState({ mures: originalMures });
			}
			console.log("Users: ", this.state.mures);
		});
	};
...
handleCheckboxChange = ({ target: { checked, value } }) => {
	...
	getDataPgnation = () => {
		const { pageSize, currentPage, mures: Mures, sortColumn, searchQuery } = this.state;
		//
		//filter maybe next time
		let filtered = Mures;
		if (searchQuery) {
			console.log(searchQuery);
			filtered = Mures.filter(
				(el) =>
		...
		const mures = paginate(sorted, currentPage, pageSize);
		return { data: mures };
	};

	render() {
		const { length: count } = this.state.mures;
		const { pageSize, currentPage, sortColumn, searchQuery } = this.state;
		// if(count === 0)  return "<p>No data available</p>";

		const { data: mures } = this.getDataPgnation();
		...
		return (
			<div>
				<ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item">
						<Link to="/homeopathy/books/mures">Tables</Link>
					</li>
					<li className="breadcrumb-item active">Data Tables</li>
				</ol>
				<h1 className="page-header">Mures </h1>
				<Panel>
					<PanelHeader>Mures Management</PanelHeader>

					<React.Fragment>
						<ToastContainer />
						<div className="toolbar" style={toolbarStyles}>
							<button className="btn btn-default active m-r-5 m-b-5" title="add Mure" style={btnStyles}>
								{" "}
								<Link to="/books/mures/new">
									<img style={iconStyles} src={newIcon} />
								</Link>
							</button>

							<button className="btn btn-default active m-r-5 m-b-5" title="edit Mure" style={btnStyles}>
								{" "}
								<Link
									to={
										this.state.checkedFields
											? `/books/mures/${this.state.checkedFields[0]}`
											: "/books/mures/"
									}
								>
