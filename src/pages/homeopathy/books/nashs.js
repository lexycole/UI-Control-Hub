...
import { getNashs } from "./../../../services/nashs";
import "bootstrap/dist/css/bootstrap.min.css";
//import FloatSubMenu from './../../components/float-sub-menu/float-sub-menu';
import Pagination from "../../../common/pagination";
import { paginate } from "../../../utils/paginate";
import NashsTable from "../../../components/nashsTable.jsx";
...
class NashsTableData extends Component {
	constructor(props) {
		super(props);
		this.state = {
			nashs: [],
	...
	async componentDidMount() {
		//const {data:Nashs} = await axios.get("http://localhost:4500/api/Nashs");
		const data = await getNashs();
		console.log(data.data);
		this.setState({ nashs: data.data });
	}

	handleDelete = (user) => {
		console.log(user);
		const Nashs = this.state.Nashs.filter((el) => el._id !== user._id);
		this.setState({ nashs: Nashs });
	};


	handleMassDelete = (CheckedFields) => {
		const originalNashs = this.state.nashs;
		CheckedFields.map(async (Nash) => {
			const Nashs = this.state.nashs.filter((Nash) => Nash._id !== Nash);
		
			this.setState({ nashs:Nashs });
			try {
				await http.delete(apiUrl + "/Nashs/" + Nash);
			} catch (ex) {
				if (ex.response && ex.response === 404) {
					alert("already deleted");
				}

				this.setState({ nashs: originalNashs });
			}
			console.log("Users: ", this.state.nashs);
		});
	};
...
handleCheckboxChange = ({ target: { checked, value } }) => {
	...
	getDataPgnation = () => {
		const { pageSize, currentPage, nashs: Nashs, sortColumn, searchQuery } = this.state;
		//
		//filter maybe next time
		let filtered = Nashs;
		if (searchQuery) {
			console.log(searchQuery);
			filtered = Nashs.filter(
				(el) =>
		...
		const nashs = paginate(sorted, currentPage, pageSize);
		return { data: nashs };
	};

	render() {
		const { length: count } = this.state.nashs;
		const { pageSize, currentPage, sortColumn, searchQuery } = this.state;
		// if(count === 0)  return "<p>No data available</p>";

		const { data: nashs } = this.getDataPgnation();
		...
		return (
			<div>
				<ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item">
						<Link to="/homeopathy/books/nashs">Tables</Link>
					</li>
					<li className="breadcrumb-item active">Data Tables</li>
				</ol>
				<h1 className="page-header">Nashs </h1>
				<Panel>
					<PanelHeader>Nashs Management</PanelHeader>

					<React.Fragment>
						<ToastContainer />
						<div className="toolbar" style={toolbarStyles}>
							<button className="btn btn-default active m-r-5 m-b-5" title="add Nash" style={btnStyles}>
								{" "}
								<Link to="/books/nashs/new">
									<img style={iconStyles} src={newIcon} />
								</Link>
							</button>

							<button className="btn btn-default active m-r-5 m-b-5" title="edit Nash" style={btnStyles}>
								{" "}
								<Link
									to={
										this.state.checkedFields
											? `/books/nashs/${this.state.checkedFields[0]}`
											: "/books/nashs/"
									}
								>
