...
import { getKents } from "./../../../services/kents";
import "bootstrap/dist/css/bootstrap.min.css";
//import FloatSubMenu from './../../components/float-sub-menu/float-sub-menu';
import Pagination from "../../../common/pagination";
import { paginate } from "../../../utils/paginate";
import KentsTable from "../../../components/kentsTable.jsx";
...
class KentsTableData extends Component {
	constructor(props) {
		super(props);
		this.state = {
			kents: [],
	...
	async componentDidMount() {
		//const {data:Kents} = await axios.get("http://localhost:4500/api/Kents");
		const data = await getKents();
		console.log(data.data);
		this.setState({ kents: data.data });
	}

	handleDelete = (user) => {
		console.log(user);
		const Kents = this.state.kents.filter((el) => el._id !== user._id);
		this.setState({ kents: Kents });
	};


	handleMassDelete = (CheckedFields) => {
		const originalKents = this.state.kents;
		CheckedFields.map(async (Kent) => {
			const Kents = this.state.kents.filter((Kent) => Kent._id !== Kent);
		
			this.setState({ kents:Kents });
			try {
				await http.delete(apiUrl + "/Kents/" + Kent);
			} catch (ex) {
				if (ex.response && ex.response === 404) {
					alert("already deleted");
				}

				this.setState({ kents: originalKents });
			}
			console.log("Users: ", this.state.kents);
		});
	};
...
handleCheckboxChange = ({ target: { checked, value } }) => {
	...
	getDataPgnation = () => {
		const { pageSize, currentPage, kents: Kents, sortColumn, searchQuery } = this.state;
		//
		//filter maybe next time
		let filtered = Kents;
		if (searchQuery) {
			console.log(searchQuery);
			filtered = Kents.filter(
				(el) =>
		...
		const kents = paginate(sorted, currentPage, pageSize);
		return { data: kents };
	};

	render() {
		const { length: count } = this.state.kents;
		const { pageSize, currentPage, sortColumn, searchQuery } = this.state;
		// if(count === 0)  return "<p>No data available</p>";

		const { data: kents } = this.getDataPgnation();
		...
		return (
			<div>
				<ol className="breadcrumb float-xl-right">
					...
					<li className="breadcrumb-item">
						<Link to="/homeopathy/books/kents">Tables</Link>
					</li>
					<li className="breadcrumb-item active">Data Tables</li>
				</ol>
				<h1 className="page-header">Kents </h1>
				<Panel>
					<PanelHeader>Kents Management</PanelHeader>

					<React.Fragment>
						<ToastContainer />
						<div className="toolbar" style={toolbarStyles}>
							<button className="btn btn-default active m-r-5 m-b-5" title="add Kent" style={btnStyles}>
								{" "}
								<Link to="/books/kents/new">
									<img style={iconStyles} src={newIcon} />
								</Link>
							</button>

							<button className="btn btn-default active m-r-5 m-b-5" title="edit Kent" style={btnStyles}>
								{" "}
								<Link
									to={
										this.state.checkedFields
											? `/books/kents/${this.state.checkedFields[0]}`
											: "/books/kents/"
									}
								>
