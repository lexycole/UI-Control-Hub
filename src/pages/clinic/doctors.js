...
class DoctorsTableData extends Component {
	...
	handleMassDelete = (CheckedFields) => {
		...
		CheckedFields.map(async (doctor) => {
			const doctors = this.state.doctors.filter((Doctor) => Doctor._id !== doctor);
		
			this.setState({ doctors });
			try {
				await http.delete(apiUrl + "/doctors/" + doctor);
			} catch (ex) {
			...
			console.log("Users: ", this.state.doctors);
		});
	};
								...
								<Link
									to={
										this.state.checkedFields
											? `/clinic/doctors/${this.state.checkedFields[0]}`
											: "/clinic/doctors/"
									}
								>
