...
class invoiceTable extends Component {
  ...
  async componentDidMount() {
    ...
    const promises = data.map(async data => {
      ...
      return { ...data, isChecked: false, userimageSrc: imagesrc, user: fullname, paidDate: moment(patient.paidDate).format('L, h:mm '), dueDate: moment(patient.dueDate).format('L, h:mm '), treatmentDate: moment(patient.treatmentDate).format('L, h:mm ') }
    })
  ...
  handleMassDelete = (CheckedFields) => {
    ...
    CheckedFields.map(async (invoice) => {
      const invoices = this.state.invoices.filter((Invoice) => Invoice._id !== invoice);
  ...
  handleDelete = async (invoice) => {
    ///delete
    const originalInvoices = this.state.invoices;
    const invoices = this.state.invoices.filter((Invoice) => Invoice._id !== invoice._id);
    this.setState({ invoices });
    try {
  ...
  handleCheckboxChange = ({ target: { checked, value } }) => {
    if (checked) {
      ...
      this.setState({ checkedFields: Fields.filter((e) => e !== value) ,invoices: newinvoices});
