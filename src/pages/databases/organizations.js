...
class OrganizationsTableData extends Component {
	constructor(props) {
		super(props);
		this.state = {
			Organizations: [],
	...
	async componentDidMount() {
		//const {data:Organizations} = await axios.get("http://localhost:4500/api/Organizations");
	...
	handleDelete = (user) => {
		console.log(user);
		const Organizations = this.state.Organizations.filter((el) => el._id !== user._id);
	...
	handleMassDelete = (CheckedFields) => {
		...
		CheckedFields.map(async (Organization) => {
			const Organizations = this.state.Organizations.filter((Organization) => Organization._id !== Organization);
		
			this.setState({ Organizations });
			try {
				await http.delete(apiUrl + "/Organizations/" + Organization);
			} catch (ex) {
			...
			console.log("Users: ", this.state.Organizations);
		});
	};
...
handleCheckboxChange = ({ target: { checked, value } }) => {
	...
	getDataPgnation = () => {
		const { pageSize, currentPage, Organizations: Organizations, sortColumn, searchQuery } = this.state;
		//
	...
	render() {
		...
		const { data: Organizations } = this.getDataPgnation();
		...
		return (
			<div>
				...
				<Panel>
					<PanelHeader>Organizations Management</PanelHeader>

					<React.Fragment>
						<ToastContainer />
						<div className="toolbar" style={toolbarStyles}>
							<button className="btn btn-default active m-r-5 m-b-5" title="add Organization" style={btnStyles}>
								{" "}
								<Link to="/databases/isurances/new">
									<img style={iconStyles} src={newIcon} />
								</Link>
							</button>

							<button className="btn btn-default active m-r-5 m-b-5" title="edit Organization" style={btnStyles}>
								{" "}
								<Link
									to={
										this.state.checkedFields
											? `/databases/isurances/${this.state.checkedFields[0]}`
											: "/databases/isurances/"
									}
								>
...
export default OrganizationsTableData;
