...
class InsurancesTableData extends Component {
	constructor(props) {
		super(props);
		this.state = {
			Insurances: [],
	...
	async componentDidMount() {
		//const {data:Insurances} = await axios.get("http://localhost:4500/api/Insurances");
		const data = await getInsurances();
		console.log(data.data);
		this.setState({ Insurances: data.data });
	}

	handleDelete = (user) => {
		console.log(user);
		const Insurances = this.state.Insurances.filter((el) => el._id !== user._id);
		this.setState({ Insurances: Insurances });
	};


	handleMassDelete = (CheckedFields) => {
		const originalInsurances = this.state.Insurances;
		CheckedFields.map(async (Insurance) => {
			const Insurances = this.state.Insurances.filter((Insurance) => Insurance._id !== Insurance);
		
			this.setState({ Insurances });
			try {
				await http.delete(apiUrl + "/Insurances/" + Insurance);
			} catch (ex) {
				if (ex.response && ex.response === 404) {
					alert("already deleted");
				}

				this.setState({ Insurances: originalInsurances });
			}
			console.log("Users: ", this.state.Insurances);
		});
	};
...
handleCheckboxChange = ({ target: { checked, value } }) => {
	...
	getDataPgnation = () => {
		const { pageSize, currentPage, Insurances: Insurances, sortColumn, searchQuery } = this.state;
		//
		...
		const Insurances = paginate(sorted, currentPage, pageSize);
		return { data: Insurances };
	};

	render() {
		const { length: count } = this.state.Insurances;
		const { pageSize, currentPage, sortColumn, searchQuery } = this.state;
		// if(count === 0)  return "<p>No data available</p>";

		const { data: Insurances } = this.getDataPgnation();
		...
		return (
			<div>
				...
				<h1 className="page-header">Insurances </h1>
				<Panel>
					<PanelHeader>Insurances Management</PanelHeader>

					<React.Fragment>
						<ToastContainer />
						<div className="toolbar" style={toolbarStyles}>
							<button className="btn btn-default active m-r-5 m-b-5" title="add Insurance" style={btnStyles}>
								{" "}
								<Link to="/databases/insurances/new">
									<img style={iconStyles} src={newIcon} />
								</Link>
							</button>

							<button className="btn btn-default active m-r-5 m-b-5" title="edit Insurance" style={btnStyles}>
								{" "}
								<Link
									to={
										this.state.checkedFields
											? `/databases/insurances/${this.state.checkedFields[0]}`
											: "/databases/insurances/"
									}
								>
...
export default InsurancesTableData;
