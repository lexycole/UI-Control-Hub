...
import { getAllens } from "./../../services/banks";
...
class BanksTableData extends Component {
	constructor(props) {
		super(props);
		this.state = {
			Banks: [],
	...
	async componentDidMount() {
		//const {data:Banks} = await axios.get("http://localhost:4500/api/Banks");
	...
	handleDelete = (user) => {
		console.log(user);
		const Banks = this.state.Banks.filter((el) => el._id !== user._id);
	...
	handleMassDelete = (CheckedFields) => {
		...
		CheckedFields.map(async (Bank) => {
			const Banks = this.state.Banks.filter((Bank) => Bank._id !== Bank);
		
			this.setState({ Banks });
			try {
				await http.delete(apiUrl + "/Banks/" + Bank);
			} catch (ex) {
			...
			console.log("Users: ", this.state.Banks);
		});
	};
...
handleCheckboxChange = ({ target: { checked, value } }) => {
	...
	getDataPgnation = () => {
		const { pageSize, currentPage, Banks: Banks, sortColumn, searchQuery } = this.state;
		//
		...
		const Banks = paginate(sorted, currentPage, pageSize);
		return { data: Banks };
	};

	render() {
		const { length: count } = this.state.Banks;
		const { pageSize, currentPage, sortColumn, searchQuery } = this.state;
		// if(count === 0)  return "<p>No data available</p>";

		const { data: Banks } = this.getDataPgnation();

		return (
			<div>
				<h1 className="page-header">Banks </h1>
				<Panel>
					<PanelHeader>Banks Management</PanelHeader>

					<React.Fragment>
						<ToastContainer />
						<div className="toolbar" style={toolbarStyles}>
							<button className="btn btn-default active m-r-5 m-b-5" title="add Bank" style={btnStyles}>
								{" "}
								<Link to="/databases/banks/new">
									<img style={iconStyles} src={newIcon} />
								</Link>
							</button>

							<button className="btn btn-default active m-r-5 m-b-5" title="edit Bank" style={btnStyles}>
								{" "}
								<Link
									to={
										this.state.checkedFields
											? `/databases/banks/${this.state.checkedFields[0]}`
											: "/databases/banks/"
									}
								>
