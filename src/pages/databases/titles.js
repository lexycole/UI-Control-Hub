...
import { getTitles } from "./../../services/insurances";
import "bootstrap/dist/css/bootstrap.min.css";
//import FloatSubMenu from './../../components/float-sub-menu/float-sub-menu';
import Pagination from "../../common/pagination";
import { paginate } from "../../utils/paginate";
import TitlesTable from "../../components/titlesTable.jsx";
...
class TitlesTableData extends Component {
	constructor(props) {
		super(props);
		this.state = {
			Titles: [],
	...
	async componentDidMount() {
		//const {data:Titles} = await axios.get("http://localhost:4500/api/Titles");
	...
	handleDelete = (user) => {
		console.log(user);
		const Titles = this.state.Titles.filter((el) => el._id !== user._id);
	...
	handleMassDelete = (CheckedFields) => {
		...
		CheckedFields.map(async (Title) => {
			const Titles = this.state.Titles.filter((Title) => Title._id !== Title);
		
			this.setState({ Titles });
			try {
				await http.delete(apiUrl + "/Titles/" + Title);
			} catch (ex) {
			...
			console.log("Users: ", this.state.Titles);
		});
	};
...
handleCheckboxChange = ({ target: { checked, value } }) => {
	...
	getDataPgnation = () => {
		const { pageSize, currentPage, Titles: Titles, sortColumn, searchQuery } = this.state;
		//
		//filter maybe next time
		let filtered = Titles;
		if (searchQuery) {
			console.log(searchQuery);
			filtered = Titles.filter(
				(el) =>
	...
	render() {
		...
		const { data: Titles } = this.getDataPgnation();
		...
		return (
			<div>
				...
				<h1 className="page-header">Titles </h1>
				<Panel>
					<PanelHeader>Titles Management</PanelHeader>

					<React.Fragment>
						<ToastContainer />
						<div className="toolbar" style={toolbarStyles}>
							<button className="btn btn-default active m-r-5 m-b-5" title="add Title" style={btnStyles}>
								{" "}
								<Link to="/databases/titles/new">
									<img style={iconStyles} src={newIcon} />
								</Link>
							</button>

							<button className="btn btn-default active m-r-5 m-b-5" title="edit Title" style={btnStyles}>
								{" "}
								<Link
									to={
										this.state.checkedFields
											? `/databases/titles/${this.state.checkedFields[0]}`
											: "/databases/titles/"
									}
								>
...
export default TitlesTableData;
